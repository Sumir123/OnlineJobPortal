...
if (isset($_POST['submit'])) {
    ...
    if ($userEmail == "" || !filter_input(INPUT_POST, "cEmail", FILTER_VALIDATE_EMAIL)) {
        $err_cEmail = "Invalid Email format";
    }
    if ($userPhone == "" || !preg_match("/^[0-9]/", $userPhone) || !preg_match("/^98[0-9]{8}$/", $userPhone)) {
        $err_cPhone = "Invalid phone number";
    }
    if ($userPassword == "" || !preg_match("/(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{8,32}/", $userPassword)) {
        ...
        <div class="form-container ">
            <form method="POST">
                <div class="form-input">
                    <label for="cName">Freelancer Name:</label>
